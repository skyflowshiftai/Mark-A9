...
class CurrencyRecognizer:
    def __init__(self):
        # Comprehensive Color Profiles for all Indian Rupee Banknotes (Mahatma Gandhi New & Existing Series)
        self.inr_profiles = {
            "2000": {
    ...
    def identify_note(self, frame_bgr: np.ndarray, language: str = "te-IN", scan_ocr: bool = False) -> Dict[str, Any]:
        """
        Analyzes color profiles, aspect ratio, and numeral patterns to recognize all Indian Rupee banknotes.
        Returns bilingual voice guidance in Telugu and English.
        """
        ...
        # 1. Check for OCR Numeral Match on Note (when requested)
        if scan_ocr:
        ...
        # 2. Color Profile Matching with distance weighting
        best_denom = None
        best_score = 0.0
        ...
        for denom, prof in self.inr_profiles.items():
            ...
            # Special case for ₹500 (Stone grey has lower saturation)
            if denom == "500" and mean_s < 70 and 65 <= mean_v <= 210:
                score += 30.0

            # Special case for ₹2000 (Magenta hues)
    ...
    def _scan_numeral(self, frame_bgr: np.ndarray) -> Optional[str]:
        """
        Attempts fast OCR numeral extraction (10, 20, 50, 100, 200, 500, 2000).
        """
        try:
            import pytesseract
